...
if ( isset( $wp_version ) && version_compare( $wp_version, '3.5' ) >= 0 ) {
	function matthewruddy_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false, $quality = false ) {
		global $wpdb;
		if ( empty( $url ) ) return new WP_Error( 'no_image_url', __( 'No image URL has been entered.' ), $url );
		...
		// Get the destination file name
			...
			if ( !file_exists( $dest_file_name ) ) {
				 ...
				 */
					...
					if ( !$get_attachment ) return array( 'url' => $url, 'width' => $width, 'height' => $height );
				...
				// Set the Quality
					if( $quality ) $editor->set_quality( $quality );
	...
	function matthewruddy_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
		...
		if ( !file_exists( $dest_file_name ) ) {
			...
			if ( !is_resource( $image ) ) return new WP_Error( 'error_loading_image_as_resource', $image, $file_path );
			
			// Get the current image dimensions and type
				$size = @getimagesize( $file_path );
				if ( !$size ) return new WP_Error( 'file_path_getimagesize_failed', __( 'Failed to get $file_path information using "@getimagesize".', 'rivasliderpro'), $file_path );
			...
			// Do some proportional cropping if enabled
				...
				else imagecopyresampled( $new_image, $image, 0, 0, 0, 0, $dest_width, $dest_height, $orig_width, $orig_height );
